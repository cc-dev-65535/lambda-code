...
def lambda_handler(event, context):
    try:
        ...
        stmt_update = text(
            """UPDATE timesheet
            SET submission_date = null, status = 'rejected', approved_date = null
            WHERE id = :id"""
        )
            ...
            Message=json.dumps(
                {
                    ...
                    "body": f"""We would like to notify you of a timesheet rejection for project: <b>{result_dicts_email[0]["name"]}</b>,
        ...
        return {
            "statusCode": "200",
            "headers": cors_headers,
            "body": json.dumps({"status": "success", "message": "Timesheet rejected"}),
        }
